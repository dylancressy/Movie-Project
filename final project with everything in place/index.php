...
if(isset($_POST['btn-login'])) {
	...
	$query = $MySQLi_CON->query("SELECT user_id, user_email, user_pass, type FROM users WHERE user_email='$email'");
...
// 		header("Location: home.php");
// 	}
// 	// else if($result == 1 AND $row['user_pass']==md5($upass)){
// // 		$_SESSION['userSession'] = $row['user_id'];
// // 		header("Location: about.html");
// // 	}
	if ( $result == 1 AND $row['user_pass'] == md5($upass) ) {

		$_SESSION["userSession"] = $row['type'];
		

		if ( $row['type'] == 0 ){
			header("Location:flixpix.php");
		} elseif ( $row['type'] == 1 ) {
